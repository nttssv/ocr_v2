...
from fastapi import FastAPI, HTTPException, Header, Query, Body, Depends, BackgroundTasks
...
async def get_cases_ready_for_extraction(
    ...
    return {
        ...
        "claimed": claim,
        "lease_expires_at": ready_cases[0]["lease_expires_at"] if claim and ready_cases else None
    }
...
async def extend_lease(
    ...
    # Extend lease
    case["lease_expires_at"] = current_time + timedelta(minutes=extension.duration_minutes)
    case["updated_at"] = current_time
...
async def get_webhook_history(limit: int = Query(default=50, ge=1, le=1000)):
    ...
    return {
        "webhooks": webhooks_db[-limit:],
        "total_count": len(webhooks_db)
    }
...
async def health_check():
    ...
    return {
        ...
        "stats": {
            ...
            "active_leases": len([c for c in cases_db.values() if c["lease_expires_at"]])
        }
    }
...
async def get_metrics():
    ...
    return {
        ...
        "active_leases": len([c for c in cases_db.values() if c["lease_expires_at"] and c["lease_expires_at"] > current_time])
    }

if __name__ == "__main__":
    uvicorn.run(
        "case_management_api:case_app",
        host="0.0.0.0",
        port=8001,
        reload=True
    )