...
from test_config import TestEnvironment, setup_test_environment, cleanup_test_environment
...
class APITester:
    ...
    def test_case_management(self) -> str:
        ...
        if not response or response.status_code != 200:
            self.log_test("Create Case", False, f"Status: {response.status_code if response else 'No response'}")
            return None
    ...
    def test_document_management(self, case_id: str) -> str:
        ...
        response = self.make_request("POST", f"/v1/cases/{case_id}/documents", json=doc_data)
        if not response or response.status_code != 200:
            self.log_test("Add Document", False, f"Status: {response.status_code if response else 'No response'}")
            return None
        ...
        if response and response.status_code == 200:
            ...
            self.log_test("List Case Documents", True, f"Found {len(documents)} documents")
        else:
    ...
    def test_job_management(self, case_id: str) -> str:
        ...
        if not response or response.status_code != 200:
            self.log_test("Create OCR Job", False, f"Status: {response.status_code if response else 'No response'}")
            return None
    ...
    def test_extraction_workflow(self, case_id: str):
        ...
        response = self.make_request("GET", "/v1/cases/ready-for-extraction", params={"claim": False, "limit": 5})
        if response and response.status_code == 200:
            ready_cases = response.json()
            self.log_test("Get Ready Cases (No Claim)", True, f"Found {len(ready_cases['cases'])} ready cases")
        else:
            self.log_test("Get Ready Cases (No Claim)", False)
        
        # 3. Claim cases for extraction
        response = self.make_request("GET", "/v1/cases/ready-for-extraction", 
                                   params={"claim": True, "lease_duration_minutes": 30, "limit": 1})
        if response and response.status_code == 200:
            claimed_cases = response.json()
            self.log_test("Claim Cases for Extraction", True, f"Claimed: {claimed_cases['claimed']}")
        else:
        ...
        update_data = {
            "status": "in_progress",
            "metadata": {"worker_id": "test_worker_001", "started_at": datetime.now().isoformat()}
        }
        response = self.make_request("PATCH", f"/v1/cases/{case_id}/extraction-status", json=update_data)
        if response and response.status_code == 200:
        ...
        response = self.make_request("PATCH", f"/v1/cases/{case_id}/lease/extend", json=extension_data)
        if response and response.status_code == 200:
    ...
    def test_bulk_operations(self, case_ids: List[str]):
        ...
        response = self.make_request("PATCH", "/v1/cases/extraction-status/bulk", json=bulk_data)
        if response and response.status_code == 200:
            results = response.json()
            successful = sum(1 for r in results["results"] if r["success"])
            self.log_test("Bulk Update Extraction Status", True, f"{successful}/{len(updates)} successful")
        else:
    ...
    def test_webhook_system(self):
        ...
        response = self.make_request("GET", "/v1/webhooks/history", params={"limit": 10})
        if response and response.status_code == 200:
            history = response.json()
            self.log_test("Get Webhook History", True, f"Found {len(history['webhooks'])} webhooks")
        else:
    ...
    def test_pagination_and_filtering(self):
        ...
        response = self.make_request("GET", "/v1/cases", params={"status": "created", "limit": 5})
        if response and response.status_code == 200:
            cases = response.json()
            self.log_test("Filter Cases by Status", True, f"Found {len(cases['cases'])} created cases")
        else:
            self.log_test("Filter Cases by Status", False)
        
        # Test job filtering
        response = self.make_request("GET", "/v1/jobs", params={"status": "pending", "limit": 5})
        if response and response.status_code == 200:
            jobs = response.json()
            self.log_test("Filter Jobs by Status", True, f"Found {len(jobs['jobs'])} pending jobs")
        else:
    ...
    def test_metrics_and_monitoring(self):
        ...
        if response and response.status_code == 200:
            ...
            self.log_test("Get System Metrics", True, f"Case statuses: {len(metrics['case_statuses'])}")
        else:
        ...
        if response and response.status_code == 200:
            ...
            self.log_test("Health Check with Stats", True, 
    ...
    def test_error_handling(self):
        ...
        job_data = {
            "case_ids": ["invalid-case-id"],
            "language": "vie"
        }
        response = self.make_request("POST", "/v1/jobs", json=job_data)
        if response and response.status_code == 400:
            self.log_test("Create Job with Invalid Case", True, "Correctly returned 400")
        else:
            self.log_test("Create Job with Invalid Case", False)
        
        # 3. Missing idempotency key
        headers = {"Content-Type": "application/json"}
        response = requests.post(f"{self.base_url}/v1/cases", 
            ...
            if response.status_code == 200:
                ...
                if os.path.exists(SAMPLE_PDF):
                    with open(SAMPLE_PDF, 'rb') as f:
                        ...
                        response = requests.post(f"{self.ocr_url}/documents/transform", 
                                               files=files, data=data, timeout=30)
                    
                    if response.status_code == 200:
                        result = response.json()
                        self.log_test("OCR Document Processing", True, 
                                     f"Document ID: {result.get('document_id')}")
                    else:
                        self.log_test("OCR Document Processing", False)
                else:
                    self.log_test("OCR Document Processing", False, f"Sample PDF not found: {SAMPLE_PDF}")
            else:
                self.log_test("OCR API Health Check", False, f"Status: {response.status_code}")
        except Exception as e:
        ...
        for i, test_case in enumerate(test_cases, 1):
            ...
            try:
                ...
                with open(test_pdf_path, 'rb') as f:
                    ...
                    response = requests.post(f"{self.ocr_url}/documents/transform", files=files, data=data)
                ...
                if response.status_code == 200:
                    ...
                    while wait_time < max_wait:
                        status_response = requests.get(f"{self.ocr_url}/documents/status/{document_id}")
                            ...
                            if status_data['status'] == 'completed':
                                ...
                                if 'result' in status_data and status_data['result']:
                                    ...
                                    if output_dir and os.path.exists(output_dir):
                                        self.log_test(test_name, True, f"Output: {output_dir}")
                                    else:
                                        self.log_test(test_name, False, f"Output directory not found: {output_dir}")
                                else:
                                    self.log_test(test_name, False, "No result data in response")
                                break
                            elif status_data['status'] == 'failed':
                                self.log_test(test_name, False, f"Processing failed: {status_data.get('error', 'Unknown error')}")
                                break
                    ...
                    self.log_test(test_name, False, f"Upload failed: {response.status_code}")
    ...
    def test_path_sanitization(self):
        ...
        test_pdf_path = SAMPLE_PDF if os.path.exists(SAMPLE_PDF) else "data/samples/1.pdf"
        if not os.path.exists(test_pdf_path):
    ...
    def run_end_to_end_workflow(self):
        ...
        try:
            ...
            self.log_test("End-to-End Workflow", True, f"Case {case_id} processed successfully")
...
def main():
    ...
    with TestEnvironment():
        print("🧪 Using dedicated test output directory to avoid cluttering production outputs")
        ...
        print("🧪 Hierarchy enhancement tests included for API output structure validation")
